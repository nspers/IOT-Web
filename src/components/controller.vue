...
export default {
  ...
  data() {
    ...
    return {
      ...
      apis: {
        iotcomm: {
          add: "/lightComCtl/add",
          delete: "/lightComCtl/delete/",
          update: "/lightComCtl/update",
          query: "/lightComCtl/find",
          columns: [
            {
              key: "id",
              title: "id"
  ...
  watch: {
    ...
    "dataModal.gatewayId"(val) {
      this.datas.loops.forEach(loop => {
        loop._hide = loop.gatewayId != val;
      });
    }
  },
  mounted: function() {
  ...
  methods: {
    init() {
      ...
      if (menu) {
        ...
        this.$api
          .post("/gateway/findByCom", {
            pageSize: 999,
            pageNum: 1
          })
          .then(function(res) {
            self.datas.gateways = res.result.list;
          });
        this.$api
          .post("/loop/findByCom", {
            pageSize: 999,
          ...
          .then(function(res) {
            self.datas.loops = res.result.list;
          });
    ...
    selectDatas(k) {
      return this.datas[k].filter(d => !d._hide);
    },
    updateList() {
      var self = this;
