<template>
  <div class="spon subMain">
    ...
    >
      <Button v-if="['led','lcd'].includes(apiName)">
        <Icon type="primary" />批量增加
      </Button>
    </Upload>
    <Table
      :loading="tabLoading"
      ...
      >
        ...
        >
          <div class="editDiv">
            ...
            />
            <i-switch v-else-if="column.ctype === 'switch'" :value="true" />
            <Input
              v-else
...
export default {
  ...
  data() {
    return {
      ...
      apis: {
        ...
        frt: {
          add: "/frt/addFrt",
          delete: "/frt/removeFrt/",
          update: "/frt/updateFrt",
          query: "/frt/findFrt",
          columns: [
            {
              key: "id",
        ...
        lcd: {
          add: "/lcd/addLcd",
          delete: "/lcd/removeLcd/",
          update: "/lcd/updateLcd",
          query: "/lcd/findLcd",
          columns: [
            {
              key: "id",
  ...
  methods: {
    handleUpload(file) {
      this.$Message.info('批量增加中...');
      setTimeout(() => {
        this.$Message.success("批量增加成功");
      }, 2000);
      return false;
    },
    init() {
      ...
      if (menu) {
        ...
        self.editColumns.forEach(col => {
          ...
          // 可为空的字段
          if (col.key === "describe" || col.ctype === "switch") {
            rv.required = false;
          }
